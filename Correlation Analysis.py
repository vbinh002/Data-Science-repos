...
# we can use the polyval function to evaluate our model over some points (100) 
# within the range of min() and max() values of our x-axis
# What this does is to simply run y = m * x + b function for all points in xp 
...
print ("Params 1: ", results.summary())
print ("---------------------------------------------")
...
print ("---------------------------------------------")
# you will notice that in these results, there is a term called Intercept added 
# to the regression relation. This is an automatic step where a constant column
# has been added as a dependent variable to mitigate biases in the regression analysis.
# Have a look at this page for a related discussion:
...
# These two give a slightly different R^2 although everything else in the summary result is the same.
# my guess is that there is a slight bug here with statsmodels in the way that R^2 is computed. 
#For the sake of compatibility, I would leave the hasconst = False
